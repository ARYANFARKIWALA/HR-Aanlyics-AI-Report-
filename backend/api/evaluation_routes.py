...
def run_evaluation(
    ...
    if (
        getattr(current_user, "role", "") not in ["admin", "hr_manager"]
        and not AuthorizationService.has_permission(db, current_user, "admin:audit")
    ):
        raise HTTPException(
...
@router.get("/latest", response_model=EvaluationSummary | None)
